...
export function BarChart({ data, color = 'var(--accent)' }) {
  ...
  const bw = Math.min(14, slot * 0.36); // schlanke Balken – filigraner, cleaner Look
  // #14: „KW" nur am ersten Balken anzeigen, danach reicht die Wochenzahl (kompakter, besser lesbar).
  const allKW = data.length > 1 && data.every((d) => typeof d.label === 'string' && d.label.startsWith('KW '));
  const lbl = (d, i) => (allKW && i > 0 ? d.label.slice(3) : d.label);
  return html`<svg class="chart" viewBox="0 0 ${W} ${H}" width="100%" preserveAspectRatio="xMidYMid meet">
    ${data.map((d, i) => {
      ...
      const y = H - padB - h;
      return html`<g>
        <rect x=${x} y=${y} width=${bw} height=${Math.max(0, h)} rx="3" fill=${color} />
        ${d.value > 0 ? html`<text x=${x + bw / 2} y=${y - 4} text-anchor="middle" class="chart-val">${d.value}</text>` : null}
        <text x=${x + bw / 2} y=${H - 8} text-anchor="middle" class="chart-lbl">${lbl(d, i)}</text>
      </g>`;
    })}
  </svg>`;
}
...
export function LineChart({ data, unit = '' }) {
  ...
  // Y-Achse an den Datenbereich anpassen (NICHT bei 0 starten) – sonst wirkt z. B. der
  // Gewichtsverlauf fast flach. Etwas Puffer ober-/unterhalb, damit die Linie nicht klebt.
  const dataMin = vals.length ? Math.min(...vals) : 0;
  const dataMax = vals.length ? Math.max(...vals) : 1;
  const span = dataMax - dataMin;
  const pad = span > 0 ? span * 0.15 : (Math.abs(dataMax) * 0.1 || 1);
  ...
  return html`<svg class="chart" viewBox="0 0 ${W} ${H}" width="100%" preserveAspectRatio="xMidYMid meet">
    ...
    ${data.map((d, i) => html`<g>
      ...
      <text x=${xAt(i)} y=${H - 8} text-anchor="middle" class="chart-lbl">${lbl(d, i)}</text>
    </g>`)}
  </svg>`;
}
